...
function updateClue(t, n) {
  ...
  switch (clueElement.Type) {
    ...
    case "Image":
      document.querySelector(
        `[id=question-template]`
      ).innerHTML = `<p>${clueElement.Clue}</p><img src=\"${clueElement.File}\" class=\"r-stretch\"></img><aside class=\"notes\" id=\"temp-notes1\">template notes 📝</aside>`;
      break;
    case "Video":
      document.querySelector(
        `[id=question-template]`
      ).innerHTML = `<p>${clueElement.Clue}</p><video id="media" type='video/mp4' src="${decodeURI(clueElement.File.replace('https://drive.google.com/open?id=','https://drive.google.com/uc?export=download&id='))}" class="r-stretch"></video><aside class="notes" id="temp-notes1">template notes 📝</aside>`;
      break;
    case "Audio":
      document.querySelector(
        `[id=question-template]`
      ).innerHTML = `<p>${clueElement.Clue}</p><br><audio controls id="media" src="${decodeURI(clueElement.File.replace('https://drive.google.com/open?id=','https://drive.google.com/uc?export=download&id='))}"></audio><aside class="notes" id="temp-notes1">template notes 📝</aside>`;
      break;
  }
