...
var YesReacts = [
  "Thumb-up.gif",
  "Ecstatic-Animation1 1 (Small).gif",
  "none-Cheer 1.gif",
  "none-Cheer.gif",
];
var NoReacts = [
  "Disappointed-Animation2 1.gif",
  "none-Angry.gif",
  "Thumb-down.gif",
];
var GameData = [];
...
function setPlayer(playerName) {
  let elem = document.getElementById("ShowPlayerName1");
  let elem2 = document.getElementById("ShowPlayerName2");
  let elem3 = document.getElementById("ShowPlayerName3");

  elem.innerHTML = playerName;
  elem2.innerHTML = playerName;
  elem3.innerHTML = playerName;

  const playersElem = document.getElementById("players");
  const noResponseElem = document.getElementById("noResponse");
  const notesElem = document.querySelector(`[id=temp-notes2]`);
  notesElem.removeChild(playersElem);
  notesElem.removeChild(noResponseElem);
  ...
  const judgeCorrectButton = document.createElement('b');
  judgeCorrectButton.setAttribute('onclick','window.opener.keySim(49)');
  judgeCorrectButton.className = 'button-10';
  judgeCorrectButton.textContent = '(1): Correct response,';
  
  const judgeIncorrectButton = document.createElement('b');
  judgeIncorrectButton.setAttribute('onclick','window.opener.keySim(48)');
  judgeIncorrectButton.className = 'button-10';
  judgeIncorrectButton.textContent = '(0): Incorrect response';
  
  const judgeButtons = document.createElement('div');
  judgeButtons.id = "judgeButtons";
  judgeButtons.append(judgeCorrectButton);
  judgeButtons.append(document.createElement('br'));
  judgeButtons.append(judgeIncorrectButton);

  notesElem.prepend(judgeButtons);
  notesElem.prepend(playerElem);

  Reveal.sync();
  
  //sync speaker view notes
  Reveal.togglePause();
  Reveal.togglePause();
}

function isEligible(playerNumber){
  // const found = Object.keys(eligiblePlayers).find(key => eligiblePlayers[key] === playerNumber);
  return Object.keys(eligiblePlayers).find(key => eligiblePlayers[key] === playerNumber);
  // console.log(found)
}
...
function getPlayer() {
  ...
  return new Promise((resolve) => {
    const listener = (e) => {
      const eligibility = isEligible(e.keyCode) ? true:false;
      console.log(eligibility)
      // if(eligibility.startsWith("Player")){
      if(eligibility){
      // if(isEligible(e.keyCode) !== 'undefined'){

        document.removeEventListener("keydown", listener);
        
        //add player name to slides
        console.log(e);
        playerName = isEligible(e.keyCode);
    ...
    const listener = (e) => {
      ...
      switch (true) {
        case e.keyCode == 49:
          ...
          document.getElementById(`score-notes`).innerHTML =
            document.getElementById(`score-table`).outerHTML;
          resolve();
          break;
        case e.keyCode == 48:
          document.removeEventListener("keydown", listener);
          const buttons = document.getElementById("judgeButtons");
          const parent = document.getElementById("temp-notes2");
          parent.removeChild(buttons);
          parent.append(document.createElement('div'))
          parent.innerHTML += noResponseButton;
          ...
          //remove player from guessing again
          delete eligiblePlayers[player]
          const playerElem = document.getElementById('player-name');
          const notesElem = document.querySelector(`[id=temp-notes2]`);
          notesElem.removeChild(playerElem);

          //update eligible players table
          notesElem.prepend(eligiblePlayersTable());
          Reveal.sync();
...
function eligiblePlayersTable(){
  playersTable = document.createElement('table')
  playersTable.id = 'players';
  playersTable.style.width = "100%";
  let row = playersTable.insertRow();
  for (const [index, [key, value]] of Object.entries(eligiblePlayers).entries()) {
    if( index === (Math.round(((Object.keys(eligiblePlayers).length)/2)-1))){
      row = playersTable.insertRow();
    }
    const cell = row.insertCell();
    const button = document.createElement('b')
    button.setAttribute('onclick',`window.opener.keySim(${value})`);
    button.className = `button-10`;
    button.textContent = key.replace('Player ', '');
    cell.append(button)
  }
  return playersTable;
}
...
const noResponseButton = `<br><b onclick= "window.opener.keySim(50)" class="button-10" id="noResponse">(2): No response</b><br>`
var clueElement;
function updateClue(t, n) {
  ...
  document
    .querySelector(`[data-question-value=notes-${clue}]`)
    .removeAttribute("class");
  document.querySelector(`[data-question-value=notes-${clue}]`).innerHTML = "";

  document
    .querySelector(`[data-question-value=${clue}]`)
  ...
  switch (clueElement.Type) {
    case "Text":
      ...
      ).innerHTML = `<h2 onclick= \"window.opener.keySim(49)\" class=\"button-10\">${clueElement.Clue}</h2><aside class=\"notes\" id=\"temp-notes1\">template notes 📝</aside>`;
      break;
    case "Image":
      document.querySelector(
        `[id=question-template]`
      ).innerHTML = `<p>${clueElement.Clue}</p>
      <iframe id="media" src="${clueElement.File.replace('https://drive.google.com/open?id=','https://drive.google.com/file/d/')}/preview" class="r-stretch"></iframe>
      <aside class=\"notes\" id=\"temp-notes1\">template notes 📝</aside>`
      //<img src=\"${clueElement.File}\" class=\"r-stretch\"></img><aside class=\"notes\" id=\"temp-notes1\">template notes 📝</aside>`;
      break;
    case "Video":
      document.querySelector(
        `[id=question-template]`
      ).innerHTML = `<p>${clueElement.Clue}</p>
      <iframe id="media" src="${clueElement.File.replace('https://drive.google.com/open?id=','https://drive.google.com/file/d/')}/preview" class="r-stretch" allow="autoplay"></iframe>
      <aside class="notes" id="temp-notes1">template notes 📝</aside>`;
      // <video id="media" type='video/mp4' src="${decodeURI(clueElement.File.replace('https://drive.google.com/open?id=','https://drive.google.com/uc?export=download&id='))}" class="r-stretch"></video><aside class="notes" id="temp-notes1">template notes 📝</aside>`;
    ...
    case "Audio":
      document.querySelector(
        `[id=question-template]`
      ).innerHTML = `<p>${clueElement.Clue}</p><br>
      <iframe id="media" src="${clueElement.File.replace('https://drive.google.com/open?id=','https://drive.google.com/file/d/')}/preview" width="640" height="480" allow="autoplay"></iframe>
      <aside class="notes" id="temp-notes1">template notes 📝</aside>`
      // <audio controls id="media" src="${decodeURI(clueElement.File.replace('https://drive.google.com/open?id=','https://drive.google.com/uc?export=download&id='))}"></audio><aside class="notes" id="temp-notes1">template notes 📝</aside>;
      break;
  }

  //document.querySelector(`[id=question-template]`).innerHTML = clueElement.Clue;
  document.querySelector(`[id=clue-selected]`).innerHTML = `<h2>${
    clueElement.Category
  }<br>${
    clueElement.Number * 100
  }</h2><aside class="notes" id="temp-notes0"><p  onclick= \"window.opener.keySim(49); passKeypress();\" class=\"button-10\">Press any key to go to the clue</p></aside>`;
  ...
  document.querySelector(`[id=YesReact]`).src =
    "outcome/" + YesReacts[Math.floor(Math.random() * YesReacts.length)];

  //update template notes
  let navOptions1 = `<b onclick= "window.opener.keySim(49); passKeypress();" class="button-10">(1): get response,</b><br> <b onclick= "window.opener.keySim(48)" class="button-10">(0): Play media</b><br> <b onclick= "window.opener.keySim(51)" class="button-10">(3): read aloud</b><br>`;
  let playerResponseButtons = `<table style="width:100%" id="players"><tr><td><b onclick= "window.opener.keySim(124)" class="button-10"> 1 </b></td><td> <b onclick= "window.opener.keySim(125)" class="button-10"> 2 </b></td><td><b onclick= "window.opener.keySim(126)" class="button-10"> 3 </b></td><td> <b onclick= "window.opener.keySim(127)" class="button-10"> 4 </b></td><td><b onclick= "window.opener.keySim(128)" class="button-10"> 5 </b></td><td> <b onclick= "window.opener.keySim(129)" class="button-10"> 6 </b></tr><tr><td><b onclick= "window.opener.keySim(130)" class="button-10"> 7 </b></td><td> <b onclick= "window.opener.keySim(131)" class="button-10"> 8 </b></td><td><b onclick= "window.opener.keySim(132)" class="button-10"> 9 </b></td><td> <b onclick= "window.opener.keySim(133)" class="button-10"> 10 </b></td><td><b onclick= "window.opener.keySim(134)" class="button-10"> 11 </b></td><td> <b onclick= "window.opener.keySim(135)" class="button-10"> 12 </b> </td></tr></table>` 
  let navOptions3 = `<b onclick= "window.opener.keySim(49)" class="button-10">(1): Back to Game Board,</b><br> <b onclick= "window.opener.keySim(48); passKeypress();" class="button-10">(0): Get more responses</b><br>`;

  document.querySelector(`[id=temp-notes1]`).innerHTML =
    navOptions1 +
    ` <b>Question:</b><p id="temp-notes1-Question">` +
    clueElement.Clue +
    "</P><b>Answer:</b></br>" +
    clueElement.Answer;
  document.querySelector(`[id=temp-notes2]`).innerHTML =
    playerResponseButtons + noResponseButton +
    " <b>Question:</b></br>" +
    clueElement.Clue +
    "</br><b>Answer:</b></br>" +
    clueElement.Answer;
  document.querySelector(`[id=temp-notes3]`).innerHTML =
    navOptions3 +
    " <b>Question:</b></br>" +
    clueElement.Clue +
    "</br><b>Answer:</b></br>" +
    clueElement.Answer;
  document.querySelector(`[id=temp-notes4]`).innerHTML =
    navOptions3 +
    " <b>Question:</b></br>" +
    clueElement.Clue +
    "</br><b>Answer:</b></br>" +
...
function previewFile(round) {
  ...
  //If there is another round, then show the next round button
  if (round + 1 < rounds.length) {
    boardNotes.innerHTML = `${
      boardNotes.innerHTML
    } <tr><p style= \"height: 80px; \"></p></tr><tr>\n\t\t\n\t<th colspan=\"5\" ><p onclick= \"window.opener.previewFile(${
      round + 1
    }); Reveal.sync()\"  class=\"button-10\">Start Round ${
      round + 1
    }</p></th></tr>`;
  }
  ...
  document.getElementById("notesCat1Description").innerHTML =
    clueElement.Description;
  ...
  document.getElementById("notesCat2Description").innerHTML =
    clueElement.Description;
  ...
  document.getElementById("notesCat3Description").innerHTML =
    clueElement.Description;
  ...
  document.getElementById("notesCat4Description").innerHTML =
    clueElement.Description;
  ...
  document.getElementById("notesCat5Description").innerHTML =
    clueElement.Description;
  ...
  document.getElementById("notesCat6Description").innerHTML =
    clueElement.Description;
  ...
  function fileToRounds() {
    ...
    console.log("clicked")
      ...
      reader.addEventListener("load", () => {
          ...
          fileString = JSON.parse(reader.result);
          console.log(fileString)
          
          fileString.forEach(round => {
            rounds.push(JSON.stringify(round))
          })
          console.log(rounds)
           previewFile(0);
        },false)
